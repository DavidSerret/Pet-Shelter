...
<!-- Newsletter Section -->
<div class="form-group" style="background-color: #f8f9fa; padding: 1.5rem; border-radius: 8px; border-left: 4px solid #17a2b8;">
    <h3 style="color: #17a2b8; margin-top: 0; margin-bottom: 1rem; font-size: 1.2rem;">
        <i data-lucide="mail"></i> Newsletter Settings
    </h3>
    
    <label class="form-check">
        <input type="checkbox" id="send_to_newsletter" name="send_to_newsletter" value="1" 
               class="form-check-input" {{ old('send_to_newsletter', $post->send_to_newsletter ?? false) ? 'checked' : '' }}>
        <span class="form-check-label" style="font-weight: 600;">Send this post to newsletter subscribers</span>
    </label>
    <small style="color: #666; display: block; margin-top: 5px; margin-bottom: 1rem;">
        When checked, this post will be sent to all newsletter subscribers.
    </small>
    
    <div id="newsletter-options" style="{{ old('send_to_newsletter', $post->send_to_newsletter ?? false) ? '' : 'display: none;' }}">
        <div class="form-group" style="margin-top: 1rem;">
            <label style="font-weight: 600; display: block; margin-bottom: 0.5rem;">Send Options</label>
            
            <div style="display: flex; gap: 2rem; margin-bottom: 1rem;">
                <label class="form-check">
                    <input type="radio" name="newsletter_send_option" value="now" 
                           class="form-check-input" 
                           {{ old('newsletter_send_option', isset($post) && $post->newsletter_scheduled_at ? 'schedule' : 'now') === 'now' ? 'checked' : '' }}>
                    <span class="form-check-label">Send immediately after saving</span>
                </label>
                
                <label class="form-check">
                    <input type="radio" name="newsletter_send_option" value="schedule" 
                           class="form-check-input" 
                           {{ old('newsletter_send_option', isset($post) && $post->newsletter_scheduled_at ? 'schedule' : 'now') === 'schedule' ? 'checked' : '' }}>
                    <span class="form-check-label">Schedule for later</span>
                </label>
            </div>
        </div>
        
        <div id="schedule-datetime" style="{{ old('newsletter_send_option', isset($post) && $post->newsletter_scheduled_at ? 'schedule' : 'now') === 'schedule' ? '' : 'display: none;' }}">
            <div class="form-group">
                <label for="newsletter_scheduled_at">Schedule Date & Time</label>
                <input type="datetime-local" id="newsletter_scheduled_at" 
                       name="newsletter_scheduled_at" class="form-control"
                       value="{{ old('newsletter_scheduled_at', isset($post) && $post->newsletter_scheduled_at ? $post->newsletter_scheduled_at->format('Y-m-d\TH:i') : '') }}"
                       min="{{ now()->format('Y-m-d\TH:i') }}">
                <small style="color: #666; display: block; margin-top: 5px;">
                    Select the date and time when you want the newsletter to be sent.
                </small>
            </div>
        </div>
        
        @if(isset($post) && $post->newsletter_sent_at)
            <div style="background-color: #d4edda; color: #155724; padding: 0.75rem; border-radius: 6px; margin-top: 1rem;">
                <i data-lucide="check-circle"></i> 
                <strong>Newsletter sent:</strong> 
                {{ $post->newsletter_sent_at->format('M j, Y \a\t g:i A') }}
            </div>
        @elseif(isset($post) && $post->newsletter_scheduled_at)
            <div style="background-color: #fff3cd; color: #856404; padding: 0.75rem; border-radius: 6px; margin-top: 1rem;">
                <i data-lucide="clock"></i> 
                <strong>Scheduled for:</strong> 
                {{ $post->newsletter_scheduled_at->format('M j, Y \a\t g:i A') }}
            </div>
        @endif
    </div>
</div>
